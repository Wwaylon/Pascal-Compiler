...
Object Converter::visitCaseStatement(PascalParser::CaseStatementContext *ctx)
{
    ...
    code.emitLine("{");
    code.indent();
    for(PascalParser::CaseBranchContext *branchIter : ctx->caseBranchList()->caseBranch()) //go to all branches
    {
        PascalParser::StatementContext *statement = branchIter->statement();
        if (branchIter->caseConstantList() == nullptr)
        {
            code.emitStart("default:");
            visit(branchIter);
        ...
        }else
        {
            code.emitStart("case");
            string temp = "";
            for(PascalParser::CaseConstantContext *caseConstantsIter : branchIter->caseConstantList()->caseConstant())
            {
                temp += (to_string(caseConstantsIter->value));
                temp += ",";
            }
            temp.pop_back();
            code.emit(" ");
...
Object Converter::visitFunctionCallFactor(PascalParser::FunctionCallFactorContext *ctx)
{
	string functionPrint = ctx->functionCall()->getText();
	//code.emitLine(functionPrint);
	return functionPrint;
}    

Object Converter::visitProcedureCallStatement(PascalParser::ProcedureCallStatementContext *ctx)
{
	string procedurePrint = ctx->procedureName()->getText();
	procedurePrint += '(';
	if (ctx->argumentList() != nullptr)
	{
		procedurePrint += ctx->argumentList()->getText();
	}
	procedurePrint += ");";
	code.emitLine(procedurePrint);
	code.emitLine();
	return nullptr;
}
